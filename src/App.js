...
const SalespersonLogin = ({ onLogin, db, appId }) => {
    ...
    const handleLogin = async () => {
        ...
        try {
            const salespersonsRef = collection(db, `/artifacts/${appId}/public/data/salespersons`);
            const q = query(salespersonsRef, where("name", "==", salesperson.trim()));
            const querySnapshot = await getDocs(q);

            if (querySnapshot.empty) {
                setError('Nom de commercial non reconnu.');
            } else {
                onLogin(salesperson.trim());
            }
        } catch (err) {
...
const AppointmentList = ({ salesperson, onNavigate, onSelectAppointment, db, appId }) => {
    const [appointments, setAppointments] = useState([]);
    const [isLoading, setIsLoading] = useState(true);

    useEffect(() => {
        if (!db || !appId || !salesperson) return;
        const appointmentsRef = collection(db, `/artifacts/${appId}/public/data/appointments`);
        const q = query(appointmentsRef, where("salesperson", "==", salesperson));
    ...
    <div className="bg-gray-100 min-h-screen font-sans p-4">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 space-y-4">
            {isLoading ? <p>Chargement...</p> : appointments.length === 0 ? <p>Aucun rendez-vous.</p> : appointments.map(app => (
            <div key={app.id} className="p-4 border rounded-lg hover:bg-gray-50 flex justify-between items-center">
              <div>
                <p className="font-bold text-lg">{app.clientName}</p>
                <p className="text-sm text-gray-600">Le {new Date(app.date).toLocaleDateString()} à {app.time}</p>
...
const NewAppointment = ({ salesperson, onBack, onAppointmentCreated, db, appId }) => {
  ...
  useEffect(() => {
    // IMPORTANT: Remplacez "VOTRE_CLE_API_GOOGLE_MAPS" par votre véritable clé API.
  ...
  const handleSave = async () => {
    ...
    const endDate = new Date(eventDate.getTime() + 60 * 60 * 1000); // Ajoute 1 heure

    const formatDateForGoogle = (d) => d.toISOString().replace(/-|:|\.\d\d\d/g, "");

    const calendarUrl = `https://www.google.com/calendar/render?action=TEMPLATE` +
                        `&text=${encodeURIComponent(`RDV - ${clientName}`)}` +
                        `&dates=${formatDateForGoogle(eventDate)}/${formatDateForGoogle(endDate)}` +
                        `&details=${encodeURIComponent(`Prospect: ${clientName}\nTéléphone: ${phone}`)}` +
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 space-y-6">
              ...
              <div>
               ...
               <div>
                  ...
                  <input ref={autocompleteInput} value={address} onChange={(e) => setAddress(e.target.value)} placeholder="123 Rue de la République..." className="w-full p-3 border rounded-lg"/>
              </div>
              <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Numéro de téléphone</label>
                  <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="0612345678" className="w-full p-3 border rounded-lg"/>
              </div>
              <button onClick={handleSave} disabled={!clientName || !date || !address || !phone} className="w-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 disabled:bg-gray-300">
...
const PresentationView = ({ onBack, db, appId }) => {
    ...
    const getEmbedUrl = (url) => {
        ...
        } catch(e) { console.error("URL invalide:", url); }
        return null;
    };
    ...
    return (
        <div className="bg-gray-100 min-h-screen font-sans p-4">
            <div className="max-w-6xl mx-auto">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-3xl font-bold text-gray-800">Vidéos de Présentation</h1>
                ...
                {isLoading ? (
                    ...
                    <p className="text-center bg-white p-8 rounded-xl shadow-lg">Aucune vidéo de présentation n'a été configurée.</p>
                ) : (
                        ...
                        {videos.map(video => {
                            ...
                            return (
                                <div key={video.id} className="bg-white rounded-xl shadow-lg overflow-hidden">
                                    <h2 className="text-xl font-bold text-gray-800 p-4">{video.title}</h2>
                                    {embedUrl ? (
                                        ...
                                        <p className="p-4 text-red-500">Le format de l'URL de cette vidéo Google Drive n'est pas valide.</p>
                                    )}
                                </div>
                            )
                        })}
...
export default function App() {
  const [currentView, setCurrentView] = useState('login'); // 'login', 'home', 'quote', 'appointmentList', 'newAppointment', 'presentation'
  const [salesperson, setSalesperson] = useState('');
    ...
    return (
      <div className="bg-gray-100 min-h-screen font-sans flex items-center justify-center p-2 sm:p-4">
        <div className="w-full max-w-2xl bg-white rounded-xl shadow-lg p-4 sm:p-8">
          <SalespersonLogin onLogin={handleLogin} db={db} appId={appId} />
        </div>
      </div>
    );
  }
  ...
  if (currentView === 'quote') {
    return <QuoteProcess data={quoteData} setData={setQuoteData} onBackToHome={handleBackToHome} db={db} appId={appId} />;
  }
  
  if (currentView === 'appointmentList') {
      return <AppointmentList salesperson={salesperson} onNavigate={setCurrentView} db={db} appId={appId} />;
  }
    
  if (currentView === 'newAppointment') {
      return <NewAppointment salesperson={salesperson} onBack={() => setCurrentView('home')} onAppointmentCreated={() => setCurrentView('appointmentList')} db={db} appId={appId}/>;
  }

  if (currentView === 'presentation') {
      return <PresentationView onBack={() => setCurrentView('home')} db={db} appId={appId} />;
  }
